...
class Instruksi_menteri extends CI_Controller {
	...
	function index()
	{
		...
		$data = array(
			'contents'	=> 'format/instruksi_menteri',
			'title'		=> 'Buat Rancangan Dokumen Instruksi Menteri',
			'sub_title'	=> 'Instruksi Menteri',
	...
	function sanusi($thisDownload = 'benar') { 
		...
		$subLevel[0]['firstLine']['indentation'] = array(
			'firstLine' => \PhpOffice\PhpWord\Shared\Converter::cmToTwip(3.25) * -1,
			'left' => \PhpOffice\PhpWord\Shared\Converter::cmToTwip(3.25),
		);
		...
		$baris_paragraf = explode("\r\n",$paragraf);

		// print_r($barisJudul); exit();
		foreach($baris_paragraf as $row_paragraf){
			if ($row_paragraf != ""){
				$txtTitle = $row_paragraf;
				$section->addText($txtTitle, $fontStyle, array('align' => 'justify'));
			} else {
				$section->addTextBreak(1,$fontStyle);
			}
		}
		$arrData['paragraf'] = $paragraf;
				...
				for($j=0; $j<count($barisMenimbang); $j++){
					...
					if ($theFirst==true){ // awal Diktum
						...
						if ($j==0){ // awal body baris Diktum
							if ($pointerMenimbang[$i] == ''){ // awal body Diktum tanpa pointer
								// echo "4";
								$section->addText(
									$barisMenimbang[$j],
									$fontStyle,
	...
	function insert_document() {
		$dataDokumen = array(
			'jenis_dokumen'	=> 'Instruksi Menteri',
