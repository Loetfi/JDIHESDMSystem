...
class Surat_perintah extends CI_Controller {
	...
	function index()
	{
		...
		$data = array(
			'contents'	=> 'format/surat_perintah',
			'title'		=> 'Buat Dokumen Rancangan Surat Perintah',
			'sub_title'	=> 'Surat Perintah',
	...
	function sanusi($thisDownload = 'benar') { 
		...
		$subLevel[1]['firstLine']['indentation'] = array(
			'firstLine' => \PhpOffice\PhpWord\Shared\Converter::cmToTwip(5.5) * -1,
			'left' => \PhpOffice\PhpWord\Shared\Converter::cmToTwip(5.5),
		);
		...
		$fontStyle['allCaps'] = TRUE;
		$judul_memerintahkan = "\r\nMEMERINTAHKAN:";
		$barisJudul_memerintahkan = explode("\r\n",$judul_memerintahkan);

		// print_r($barisJudul); exit();
		foreach($barisJudul_memerintahkan as $row){
			if ($row != ""){
				$txtTitle = $row;
				$section->addText($txtTitle, $fontStyle, array('align' => 'center'));
			} else {
		...
		$arrData['judul'] = $judul_memerintahkan;
		...
		// print_r($nextPageMemerintahkan); die();
		for($i=0; $i<count($Memerintahkan); $i++){

			if ($Memerintahkan[$i] != ''){
				$barisMemerintahkan = explode("\r\n",$Memerintahkan[$i]);

				if (@$nextPageMemerintahkan[$i] == 'newP')
					$section = $phpWord->addSection($thisPage);
				else if (@$nextPageMemerintahkan[$i] == 'newL')
					$section = $phpWord->addSection($thisPageLandscape);

				// print_r($Memerintahkan[$i]);
				// print_r($barisMemerintahkan);
				for($j=0; $j<count($barisMemerintahkan); $j++){

					$idxSubLevel = @$subLevelMemerintahkan[$i];

					if ($theFirst==true){ // awal Memerintahkan
						if ($j==0){ // awal baris Memerintahkan

							if ($pointerMemerintahkan[$i] == ''){ // awal tanpa pointer
								$section->addText(
									"Kepada\t:\t".$barisMemerintahkan[$j],
									$fontStyle,
									$subLevel[0]['firstLine']
								);
							}
								...
								$section->addText(
									"Kepada\t:\t".$pointerMemerintahkan[$i]."\t".$barisMemerintahkan[$j],
									$fontStyle,
									$subLevel[1]['firstLine']
								);
							}
							...
							if ($pointerMemerintahkan[$i] == ''){ // lanjutan tanpa pointer
								$section->addText(
									$barisMemerintahkan[$j],
									$fontStyle,
									$subLevel[0]['default']
								);
							}
					...
					else { // body Memerintahkan
						if ($j==0){ // awal body baris Memerintahkan
							if ($pointerMemerintahkan[$i] == ''){ // awal body Memerintahkan tanpa pointer
								$section->addText(
									$barisMemerintahkan[$j],
									$fontStyle,
									$subLevel[$idxSubLevel]['default']
							...
							else { // awal body Memerintahkan ada pointer
								$section->addText(
									$pointerMemerintahkan[$i]."\t".$barisMemerintahkan[$j],
									$fontStyle,
									$subLevel[$idxSubLevel]['pointer']
								);
			...
			$arrData['Memerintahkan']['nextPageMemerintahkan'][] = $nextPageMemerintahkan[$i];
		...
		*/
		...
		try {
			$filename = @$id.'_'.time().'_surat_perintah.docx';
			$fullPath = './'.$filename;
