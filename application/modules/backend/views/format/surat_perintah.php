<script>
	...
	function addMemerintahkan(ele, count) {
		var counts = count + 1;
		var element =	'<div class="form-group" id="text-Memerintahkan'+counts+'" style="display:inline-block; margin-top:5px; width:100%;">'+
		'<div style="float:left; width:20%">'+
		'<input type="text" name="pointerMemerintahkan[]" class="form-control" placeholder="Pointer" data-toggle="tooltip" title="" data-original-title="contoh : a. ( menggunakan titik dibelakang )"/>'+
		'<select name="nextPageMemerintahkan[]" class="form-control" style="cursor:pointer; margin-top:10px;">'+
		'<option value="continue">Continues Page</option>'+
		'<option value="newP">Next Page Portrait</option><option value="newL">Next Page Landscape</option>'+
		'</select>'+
		'</select>'+
		'<input type="hidden" name="subLevelMemerintahkan[]" value="0">'+ 
		'</div>'+
		'<div style="width:100%">'+
		'<textarea type="text" name="Memerintahkan[]" class="form-control" placeholder="Memerintahkan" rows="6" style="float:left; margin-left:5px; width:75%;"></textarea>'+
		'<a href="javascript:void(0)" onclick="addMemerintahkan(this, '+counts+')" style="position:absolute; right:2%;" title="Tambah Isi" class="btn btn-success btn-xs"><i class="fa fa-lg fa-plus"></i></a>'+
		'</div>'+
		'</div>';
		$(ele).children().toggleClass('fa-plus fa-minus');
		$(ele).children().attr("onclick","remove(this)").attr('title', 'Hapus Isi');
		$(element).last().insertAfter($("#text-Memerintahkan"+count));
	}
		...
		<div class="padding">
			<div class="box">
				<div class="box-header light lt">
					<h3>Rancangan Surat Perintah</h3>
					<small>Membuat sebuah dokumen Surat Perintah</small>
				</div>
				<div class="padding"> 
					<form class="width-100p" method="POST" id="form-keputusan" target="_blank" enctype="multipart/form-data">
						...
						<hr>
						<fieldset>
							<center><p>MEMERINTAHKAN:</p></center>
							<p>Kepada</p>
							<div>
								<?php $namaMemerintahkan = 'Memerintahkan';?>
								<div class="form-group" id="text-<?php echo $namaMemerintahkan; ?>1" style="display:inline-block; width:100%;">
									<div style="float:left; width:20%"> 
										<input type="text" name="pointer<?php echo $namaMemerintahkan; ?>[]" class="form-control" placeholder="Pointer" data-toggle="tooltip" title="contoh : a. ( menggunakan titik dibelakang )"/>
										<select name="nextPage<?php echo $namaMemerintahkan; ?>[]" class="form-control" style="cursor:pointer; margin-top:10px;" data-toggle="tooltip" title="Jika dalam kondisi dokumen harus menggunakan halaman baru ( potrait / lanskap ) gunakan fitur ini. Continues Page menjadi bagian default dari pengaturan ini.">
											<option value="continue">Continues Page</option>
											<option value="newP">Next Page Portrait</option><option value="newL">Next Page Landscape</option>
										</select>
										<input type="hidden" name="subLevel<?php echo $namaMemerintahkan; ?>[]" value="0">
									</div>
									<div style="width:100%">
										<textarea data-toggle="tooltip" title="sistem akan memberikan otomatis simbol ; ( titik koma) pada akhir paragraf, apabila sistem tidak memberi simbol ; maka kamu harus menambahkannya. " type="text" name="<?php echo $namaMemerintahkan; ?>[]" class="form-control" placeholder="Memerintahkan" rows="6" style="float:left; margin-left:5px; width:75%;" required></textarea>
